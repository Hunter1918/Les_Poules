    ...
    void Update()
    {
        ...
        else if (state == "mate")
        {
            ...
            else
            {
                ...
                // Destroy target
                if (Vector3.Distance(transform.position, target.transform.position) <= 1 && target.tag == "Poules")
                {
                    energy -= 40f;
                    state = "find_prey";
                    ...
                    for (int i = 0; i < reproduction; i++)
                    {
                        ...
                        newFox.GetComponent<Poules>().speed = (speed + target.GetComponent<Poules>().speed) / 2;
                        newFox.GetComponent<Poules>().energy = 30f;
                    }
                }
            }
        }
